...
def put(key, new_value):
  ...
  present, then it returns 403, otherwise, it returns 200"""
    ...
    # According to https://sourcegraph.com/github.com/boto/boto/symbols
    # /python/boto/opsworks/exceptions/ValidationException
    # ValidationException inherits from JSONResponseError
    except JSONResponseError:
        print("Something went wrong updating the database.  "
              "ValidationcwException was\
raised.")
...
if __name__ == "__main__":
    ...
    def test_post(key, value):
        ...
        if key not in check_dict:
            if DEBUG:
                assert status == 200, \
                    "Key %s not in check_dict and should be because DEBUG is " \
                    "True." + \
                    "Status is %d" % (key, status)
            else:
                assert status == 403 or status == 200, \
                    "key % is not in check_dict, and we don't know if it " \
                    "should be because" + \
    ...
    def test_delete(key):
        ...
        if key in check_dict:
            ...
            assert status == 200, \
                "Status was %d should have been 200 when removing a key that " \
                "exists" % status
            del check_dict[key]
            ...
            assert status == (
                None,
                403), "Status was %s should have been a tuple (None,403)" % \
                      str(status)
        else:
            print("The key was not in the database")
            assert status == 200, "Status was %d should have been 200 after " \
                                  "attempting to " + \
                                  "delete a key from the database that should" \
                                  " not have been there"
    ...
    def test_put(key, value):
        ...
        if key in check_dict:
            ...
            # A little consistency check to make sure that the database and
            # the dictionary
            # really are in sync.
            value = get(key)
            assert value[0] == None
    ...
    if DEBUG:
        ...
        while True:
            ...
            try:
                ...
                print(
                    "The table %s still isn't deleted.... waiting" % TABLE_NAME)
            else:
                break
        ...
        while True:
            try:
                ...
                print(
                    "Trying to insert a key-value pair failed.  Perhaps the "
    ...
    try:
        ...
        test_get(
            "Randall")  # This should throw an error because Randall is in
        # the check_dict
    except AssertionError:
        print(
            "Threw an expected Assertion error getting a non-existant key - "
            "all is well")
    else:
    ...
    try:
        ...
        print(
            "Threw an expected Assertion error - all is well.  Tried to "
            "update a non-existant key")
    else:
        assert True, "Did *not* throw an expected Assertion Error"
    # GET, HEAD, PUT
    test_delete("Janie")
    ...
    while True:
        ...
        if op == "I":
            ...
            try:
                ...
                print(
                    "Probably the key %s already has a value.  let's see" % key)
                value = get(key)
            ...
            try:
                ...
                print(
                    "Probably the key %s already doesn't a value.  let's see"
                    % key)
                try:
                    value_1 = get(key)
                except boto.dynamodb2.exceptions.ConditionalCheckFailedException:
                    ...
                    print("Something else must be wrong.  Key %s has value "
                          "%s" % \
                          (key, value_1))
        elif op == "D":
            delete(key)
